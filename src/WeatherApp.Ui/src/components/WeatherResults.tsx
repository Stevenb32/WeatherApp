...
function WeatherResults({ weather }: WeatherResultsProps) {
  ...
  return (
    <div className="rounded-3xl border border-sky-100 bg-white p-6 shadow-sm sm:p-8">
      ...
      <HourlyForecast hourly={weather.hourly} unitSystem={weather.unitSystem} timeZoneId={weather.location.timeZoneId} />

      <DailyForecast daily={weather.daily} unitSystem={weather.unitSystem} />
    </div>
  );
}
